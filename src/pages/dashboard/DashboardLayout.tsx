...
const ProfileWithModal = ({ doctorProfile, isModalOpen, setIsModalOpen }) => {
  ...
  return (
    <>
      <Button variant="ghost" className="flex items-center gap-2" onClick={handleOpenModal}>
        <Avatar className="h-8 w-8">
          <AvatarImage src={doctorProfile.avatar} />
          <AvatarFallback>{doctorProfile.name?.substring(0, 2) || "JD"}</AvatarFallback>
        </Avatar>
        <span className="font-medium text-sm hidden lg:inline-block">{doctorProfile.name}</span>
              ...
              >
                <div className="p-6 border-b h-full overflow-y-auto">
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <p className="font-medium">{user.name || "Dr. John Doe"}</p>
                      <p className="text-sm text-muted-foreground">{user.department || "Infectious Disease"}</p>
                    </div>
                  </div>

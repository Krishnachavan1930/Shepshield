...
export const getPatientProgress = async (
  ...
  try {
    ...
    console.log(
      "Patient (raw):",
      patient ? patient.toJSON() : "No patient found"
    );
    ...
    const progressData = labResults.map((lab) => {
      const vital = vitalSigns.length
        ? vitalSigns.find(
            (v) =>
              v.recordedAt &&
              lab.recordedAt &&
              new Date(v.recordedAt).getTime() ===
                new Date(lab.recordedAt).getTime()
      ...
      return {
        ...
        lactate: lab.Lactate ?? 0,
        antibiotics:
          patient && patient.medications && patient.medications !== "None"
            ? patient.medications
            : "N/A",
      };
    });
...
export const uploadCSV = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      await processCSV(req.file.path, uploadedBy);
...
const processPatients = async (
  ...
  for (const record of data) {
    const existingPatient = await Patient.findOne({
      where: { medicalRecordNumber: record.medicalRecordNumber },
    });
    if (existingPatient) {
      await existingPatient.update({ ...record, updatedBy: uploadedBy });
    } else {
      await Patient.create({ ...record, createdBy: uploadedBy });
    }
  }
};

const processVitalSigns = async (data: VitalSignRecord[]): Promise<void> => {
  await VitalSigns.bulkCreate(data as any);
};

const processLabResults = async (data: LabResultRecord[]): Promise<void> => {
  await LabResult.bulkCreate(data as any);
};
